...
Conversión a Skill de `.github/agents/BMO-Explorer.agent.md` (preservado sin cambios en esa
ruta). Todas las reglas originales se mantienen. Tiene **dos modos**:
...
1. Leer `.github/skills/katalon-mobile-automation/SKILL.md` (fuente de verdad del formato
   `.rs` y POM 3 capas). Si hay conflicto con este skill, prevalece el SKILL.
2. Si el plan involucra eventos Ads → cargar skill `qa-rads-tracker` y agregar
   `curl -s http://127.0.0.1:8082/health` al chequeo de viabilidad en MODO VALIDACIÓN.
3. Leer el archivo de contexto en `.github/orchestrator/runs/<run-id>-<flujo>.md`:
...
- Fuente original preservada: `.github/agents/BMO-Explorer.agent.md`
- Detalle exhaustivo de formato `.rs` y protocolo de captura: ya documentado en
  `.github/skills/katalon-mobile-automation/SKILL.md` — este skill no lo duplica, solo
