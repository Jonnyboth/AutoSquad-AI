...
|---|---|---|
| `UtilsPage` | `Keywords/com/rappi/page/common/UtilsPage.groovy` | El error ocurre en scroll o validación de múltiples elementos — reemplazar implementaciones ad-hoc por `scrollToElement()`/`validateElements()` |
| `DeviceResolutionPage` | `Keywords/com/rappi/page/common/DeviceResolutionPage.groovy` | El error ocurre por coordenadas absolutas sin escalar — usar `scaleX/scaleY/scalePoint()`; llamar `invalidateCache()` si cambió el dispositivo mid-sesión |
| `SmartWaitPage` | `Keywords/rappi/utils/SmartWaitPage.groovy` | Timeout de elemento: si falla con `SHORT` pero pasa con `MEDIUM` → problema de red, no de locator |
| `LocatorHelper` | `Keywords/rappi/utils/LocatorHelper.groovy` | `NoSuchElementException` — usar `findWithFallback()` para identificar qué estrategia sigue funcionando |
| `ScreenshotPage` | `Keywords/rappi/utils/ScreenshotPage.groovy` | Estado de UI inesperado — comparar pantalla actual vs baseline (`captureAndCompare`, `updateBaseline`) |
| `VisualLocatorPage` | `Keywords/rappi/utils/VisualLocatorPage.groovy` | Último recurso cuando todas las estrategias XML fallan (solo Android) |
