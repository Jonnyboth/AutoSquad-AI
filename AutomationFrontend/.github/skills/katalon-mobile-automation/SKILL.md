...
All utility classes live at `Keywords/rappi/utils/` and are available as `CustomKeywords` in any test or keyword file.

### 1. SmartWaitPage — Standardized Waits
**Package:** `rappi.utils.SmartWaitPage`
...
**Package:** `rappi.utils.LocatorHelper`
...
**Package:** `rappi.utils.ScreenshotPage`
...
```groovy
import rappi.utils.ScreenshotPage
...
**Package:** `rappi.utils.VisualLocatorPage`
...
|---|---|---|---|---|---|---|---|---|
| btn_foo | com.grability.rappi:id/foo | Foo | [100,200][300,400] | 200 | 913 | ✅ true | ACCESSIBILITY | ANDROID_UI_AUTOMATOR |
...
| `G_AppBundleID` | App package/bundle ID | `'com.grability.rappi'` |
...
3. If app is not open:
   mobile_launch_app(device="<deviceId>", packageName="com.grability.rappi")
...
```bash
python3 "/Users/jhonsebastianrianoramirez/Katalon Studio/testAndroid/.github/skills/katalon-mobile-automation/extract_video_frames.py" \
...
- ffmpeg is installed: `/opt/homebrew/bin/ffmpeg`
...
```
.github/skills/katalon-mobile-automation/extract_video_frames.py
```
...
```bash
python3 "/Users/jhonsebastianrianoramirez/Katalon Studio/testAndroid/.github/skills/katalon-mobile-automation/extract_video_frames.py" "RUTA_VIDEO" --fps 1
```
...
### Practical lessons learned (from real Rappi flow)
...
2. Page layer:
   - Keywords/com/rappi/page/android/<Screen>Page.groovy
3. Steps layer:
   - Keywords/com/rappi/steps/android/<Screen>Steps.groovy
4. Script layer:
...
**Android-specific** → `Keywords/com/rappi/page/android/<Screen>Page.groovy`
**iOS-specific** → `Keywords/com/rappi/page/ios/<Screen>Page.groovy`
**Cross-platform** → `Keywords/com/rappi/page/common/<Screen>Page.groovy`

### Step 3 — Create the Steps class (@Keyword)

**Same platform split** → `Keywords/com/rappi/steps/android/<Screen>Steps.groovy`
...
⚠️ **Path mirroring obligatorio:** Si el TC está en `Test Cases/android/TurboStore/TC_Foo.tc`, el script DEBE estar en `Scripts/android/TurboStore/TC_Foo/Script<timestamp>.groovy`. Usar el run-id del pipeline como nombre de carpeta es un error — siempre usar el nombre del TC.
...
Page classes encapsulate raw UI interactions. They live in `Keywords/com/rappi/page/`.
...
```groovy
package com.rappi.page.android   // or .ios or .common
...
Steps classes expose Page actions as Katalon Custom Keywords. They live in `Keywords/com/rappi/steps/`.
...
- Calling convention from scripts: `CustomKeywords.'com.rappi.steps.android.ExampleSteps.methodName'()`
- **NO sobrecargas (@Keyword) con mismo nombre**: Katalon Studio regenera `Libs/CustomKeywords.groovy` y produce `def static "x.y.method"` duplicado → Eclipse JDT no resuelve clases en cascada (95+ errores en Problems panel). Si necesitas variantes, usa nombres distintos (`openRappiApp`, `openRappiAppWith`).
- **NO `import internal.GlobalVariable` en Keywords**: cuando Katalon compila el Keyword antes que `internal/GlobalVariable.groovy`, lanza `NoClassDefFoundError: internal/GlobalVariable` al ejecutar desde el IDE. Resuelve por reflexión: `Class.forName('internal.GlobalVariable').getDeclaredField(name).get(null)`. Ver `RappiAppSteps.openRappiApp()` como referencia.

### Logging de aserciones — patrón obligatorio

Cualquier Steps que haga un `assert*` (validación crítica) debe usar `rappi.utils.AssertLogger` para imprimir banners ASCII visibles en el Log Viewer de Katalon. Esto estandariza el output entre todos los TCs y permite escanear los logs a simple vista.

```groovy
import rappi.utils.AssertLogger
...
@Keyword
...
`pass()` invoca `KeywordUtil.markPassed()` internamente, así que el step queda ✅ en el reporte. Para eventos con listas (`items[]` de conversion, productos de carrito, etc.) usar `AssertLogger.logItems('header', listaDeMaps, ['clave1','clave2'])`. Referencia completa: ver `Keywords/com/rappi/steps/android/RadsTrackerSteps.groovy`.
...
CustomKeywords.'com.rappi.steps.android.SomeSteps.assertSomething'()
```
...
- `com.rappi.steps.android.RadsTrackerSteps.assertClickBannerEvent() (0.014s)` ← la ejecución del assert
...
```groovy
package com.rappi.steps.android   // or .ios or .common

import com.kms.katalon.core.annotation.Keyword
import com.kms.katalon.core.mobile.keyword.MobileBuiltInKeywords as Mobile
import com.kms.katalon.core.util.KeywordUtil
import com.rappi.page.android.ExamplePage
...
El path del script debe **espejarse exactamente** desde la ubicación del Test Case. Katalon vincula un `.tc` con su script buscando la carpeta de igual nombre bajo `Scripts/`.

| Test Case path | Script path correcto |
|----------------|---------------------|
| `Test Cases/android/TC_Foo.tc` | `Scripts/android/TC_Foo/Script<timestamp>.groovy` |
| `Test Cases/android/TurboStore/TC_Bar.tc` | `Scripts/android/TurboStore/TC_Bar/Script<timestamp>.groovy` |
| `Test Cases/android/Geant/TC_Baz.tc` | `Scripts/android/Geant/TC_Baz/Script<timestamp>.groovy` |
...
### Rules
- ONLY call `CustomKeywords.'com.rappi.steps.*'()`
...
// ─── PRECONDICIÓN ─────────────────────────────────────────────
Mobile.comment('Inicia la app de Rappi')
Mobile.startExistingApplication(GlobalVariable.G_AppBundleID)

Mobile.comment('Cierra pantallas intermedias')
CustomKeywords.'com.rappi.steps.common.HomeSteps.skipScreensHome'()

// ─── PASOS DEL TEST ───────────────────────────────────────────
Mobile.comment('Navega al Help Center')
CustomKeywords.'com.rappi.steps.common.HomeSteps.goToHelpCenter'()

Mobile.comment('Selecciona la primera orden')
CustomKeywords.'com.rappi.steps.android.HelpCenterSteps.selectFirstOrder'()
...
**Fix**: `@Keyword` must be in Steps class. Verify the fully-qualified path: `'com.rappi.steps.android.ExampleSteps.methodName'`.
...
WebUI.callTestCase(findTestCase('Test Cases/android/openRappi'), [:])

// ❌ import no-estático — funciona pero NO es el estándar del SKILL
import com.kms.katalon.core.testcase.TestCaseFactory
WebUI.callTestCase(TestCaseFactory.findTestCase('Test Cases/android/openRappi'), [:])
```
...
### Cuándo usarlo
- El script necesita ejecutar `openRappi` (login/startup) antes de su flujo propio.
...
### Patrón correcto
```groovy
// Delegar startup al test openRappi
Mobile.comment('Abrir Rappi (delega a Test Cases/android/openRappi)')
WebUI.callTestCase(findTestCase('Test Cases/android/openRappi'), [:], FailureHandling.STOP_ON_FAILURE)

// Continuar con el flujo propio
CustomKeywords.'com.rappi.steps.android.SuperSteps.prepareHomeAndDismissPopup'()
```
