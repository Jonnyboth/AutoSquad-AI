...
description: >
  Planifica pruebas automatizadas Katalon Mobile/Web para Rappi ANTES de escribir código.
  Explora el flujo en dispositivo real (o navegador), identifica el punto de entrada óptimo
  reutilizando test cases ya existentes (openRappi, OpenStoreGeant, UtilsPage), valida
...
Conversión a Skill de `.github/agents/BMO-FlowPlanner.agent.md` (preservado sin cambios en esa
ruta como referencia/rollback). Todas las reglas originales se mantienen; este documento las
reorganiza para consumo por el Agente Orquestador (`qa-orchestrator`).
...
2. Leer `.github/skills/katalon-mobile-automation/SKILL.md` — fuente de verdad del formato
...
4. Si el flujo involucra eventos Ads (`rads-tracker`, banners patrocinados, métricas
   render/viewed_impression/click/add_to_cart/conversion) → cargar el skill `qa-rads-tracker`
   y aplicar su pre-flight (`curl -s http://127.0.0.1:8082/health`) antes de planificar asserts.
...
empieza en Home → reutilizar `openRappi`; si empieza dentro de Geant → `OpenStoreGeant`;
...
- Fuente original preservada: `.github/agents/BMO-FlowPlanner.agent.md`
