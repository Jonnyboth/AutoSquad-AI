...
device:
  ...
  appPackage: com.grability.rappi      # ← cambiar si pruebas otra app
  appActivity: com.grability.rappi.MainActivity   # ← cambiar para otra app
  noReset: true
  fullReset: false
...
<GlobalVariableEntity>
   <initValue>'com.grability.rappi'</initValue>     <!-- G_AppBundleID -->
...
> **Importante:** `runner.yml > device.udid` y `default.glbl > G_DevicesName` deben coincidir. Si no, los keywords que invocan `adb -s <udid>` (ej: `RappiAppPage.launchRappiViaAdb`) apuntarán a otro device.
...
La app Rappi usa Jetpack Compose y el bottom-nav **no** expone los labels en la jerarquía de vistas Android. El locator de `lbl_inicioTab.rs` apunta a un texto que solo aparece en el Home real (`text="Beneficios Pro"`). Si Rappi rebranda esa card, actualiza el atributo `text` en `Object Repository/android/Home/lbl_inicioTab.rs` con un texto Home-only verificado con `uiautomator dump` (ej.: `Turbo`, `Súper`, otra card vertical).
...
### Test pasa openRappi pero falla en pasos posteriores con cart no vacío
...
- Manualmente en device: abrir Rappi → ir a Canasta → vaciar.
...
Si `runner.yml > device.appActivity` apunta a un activity que no existe en la app, Appium se cuelga esperándola y termina con timeout. Para Rappi la actividad real es `com.rappi.discovery.onboarding.impl.activities.RappiMainEntryActivity`. Verifica en cualquier momento con la app abierta:
```bash
...
device:
  ...
  fullReset: true
```
o manualmente: `adb shell pm clear com.grability.rappi`.
...
7. `bash runner/run.sh openRappi` para validar el setup.
