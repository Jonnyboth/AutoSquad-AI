...
def _build_case_payload(test_case: dict) -> dict:
    ...
    # Campos ya en formato nativo de AIO Tests (paso directo, sin traducir).
    for passthrough in ("status", "type", "scriptType", "tags", "customFields"):
        if passthrough in test_case and passthrough not in payload:
            payload[passthrough] = test_case[passthrough]
